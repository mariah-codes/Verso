    ...
    <div className="min-h-screen bg-background pb-16">
      ...
      <div className="space-y-8 px-5">
        ...
        >
          {reading.length === 0 ? (
            <EmptyState
              message="Nothing on the nightstand yet — search a book to add one."
              href="/search"
              linkLabel="Search books"
            />
          ) : (
            <HorizontalScroll>
              {reading.map((book) => (
        ...
        >
          {wantToRead.length === 0 ? (
            <EmptyState
              message="Your reading list is empty — add books you're curious about."
              href="/search"
        ...
        >
          {finished.length === 0 ? (
            <EmptyState
              message="No finished books yet. Mark a book as read to see it here."
              href="/search"
              linkLabel="Find your first book"
            />
          ) : (
        ...
        >
          <div className="rounded-xl border border-dashed border-border px-4 py-5 text-center space-y-1">
            ...
            <p className="text-xs text-foreground/30">
              Reading streaks, first book, 10-book club…
            </p>
          </div>
        </Section>
        ...
        <Section
          ...
          count={dnf.length > 0 ? dnf.length : undefined}
        >
          <div className="rounded-xl border border-dashed border-border px-4 py-5 text-center space-y-1">
            <p className="text-sm text-foreground/40 font-sans">
              DNF shelf coming on Day 9
            </p>
            {dnf.length > 0 && (
              <p className="text-xs text-foreground/30">
                {dnf.length} book{dnf.length !== 1 ? "s" : ""} set aside
              </p>
            )}
          </div>
        </Section>
